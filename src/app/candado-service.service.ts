...
export class CandadoServiceService {
  ...
  public getPerfil(idregistro:any){

    this.descripcion= [];
    this.firestore.collection('perfil').doc(idregistro).get().subscribe
        (doc=>{

            if (!doc.exists) {
              //this.descripcion='Rol no asignado';
            } else {
              //console.log(doc.data().descripcion);
              this.descripcion.push(doc.data());
            }
